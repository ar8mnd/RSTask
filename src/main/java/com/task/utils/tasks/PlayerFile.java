                ...
                for (ItemClass itemClass : item.getItem()) {
                    if (itemClass != null) {
                        ...
                        player.sendMessage(RsTask.getTask().getLag("add-item-message")
                                .replace("%s", itemClass.getItem().getId()).replace("%c", itemClass.getItem().getCount() + ""));
                    }
            ...
            if (item.getCmd() != null && item.getCmd().length > 0) {
                for (CommandClass commandClass : item.getCmd()) {
                    if (commandClass != null) {
                        ...
                        Server.getInstance().getCommandMap().executeCommand(new ConsoleCommandSender(), commandClass.getCmd().replace("%p", playerName));

    ...
    public static SuccessItem toSuccessItem(String string) {
        ...
        switch (string.split("@")[1]) {
            case "item":
                ...
                String[] lists = sItem.split(":");
                return new SuccessItem(new ItemClass[]{
                        new ItemClass(lists[0],
                                Integer.parseInt(lists[1]),
        ...
        if (classes != null && classes.length > 0) {
            for (ItemClass itemClass : classes) {
                if (itemClass != null) {
                    ...
                    builder1.append(itemClass.getItem().getId()).append("*").append(itemClass.getItem().getCount());
